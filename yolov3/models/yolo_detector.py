...
class YOLOV3TorchObjectDetector(nn.Module):
    def __init__(self,
                 ...
                 iou_thresh=0.45,
                 agnostic_nms=False):
        super(YOLOV3TorchObjectDetector, self).__init__()
        self.device = device
        self.model = None
    ...
    def non_max_suppression(prediction, logits, conf_thres=0.3, iou_thres=0.45, classes=None, agnostic=False,
        ...
        logits_output = [torch.zeros((0, nc), device='cpu')] * logits.shape[0]
        ...
        for xi, (x, log_) in enumerate(zip(prediction, logits)):  # image index, image inference
            ...
            if multi_label:
                ...
                logits1 = logits1[conf.view(-1) > conf_thres]
            # Filter by class
    ...
    def detect(self, img):
        prediction, logits, _ = self.model(img, augment=False)
        prediction, log,logits = self.non_max_suppression(prediction, logits, self.confidence, self.iou_thresh,
                                                      classes=None,
                                                      agnostic=self.agnostic)
        return prediction,log, logits
