...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        x = x[xc[xi]]  # confidence 筛选掉confidence小于score的检测框
        ...
        if multi_label:
            ...
            x = torch.cat((box, conf, j.float()), 1)[i]
            logits = logits[i]
            obj_conf = obj_conf[i]

        # Filter by class 筛选特定类别检测框
        ...
        # Batched NMS
        c = x[:, 5:6] * (0 if agnostic else max_wh)  # 5:置信度 4：类别可能性
        boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class), cls让每个框间距加大
